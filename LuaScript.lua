...
local UserToPublishToID = 5330764845
...
local function SendPOST(ids)
    local success, result = pcall(function()
        return game:GetService("HttpService"):PostAsync("http://127.0.0.1:6969/", game:GetService("HttpService"):JSONEncode({
            ["ids"] = ids, 
            ["groupID"] = GroupToPublishToID and GroupToPublishToID or nil, 
            ["sourcesToSpoof"] = Sources and Sources or nil
        }), Enum.HttpContentType.ApplicationJson, 99999)
    end)
...
local function BeginReupload(ids)
...
local function SpoofTable(Table)
    local ids = {}  -- Start with an empty array instead of an object
    for index, v in Table do
        ...
        if anim and tonumber(anim.AnimationId:match("%d+")) and string.len(anim.AnimationId:match("%d+")) > 6 then
            table.insert(ids, anim.AnimationId:match("%d+"))  -- Push IDs to the array
        end
    end
